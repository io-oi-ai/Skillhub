...
PRD（产品需求文档）是产品经理与研发、设计、测试之间最重要的沟通工具。一份结构清晰、逻辑完整的 PRD 能显著降低需求误解与返工成本。本技能帮助产品经理快速生成结构完善的 PRD 文档。
...
- **目标用户**：该功能服务的用户群体
- **核心场景**：用户在何时、如何使用该功能
- **功能描述**：要实现的关键功能点（可先粗略描述）
- **参考产品**：如参考竞品功能，请提供链接或截图
- **优先级与时间线**：期望上线时间与功能优先级
...
3. **用户故事**：使用“作为[角色]，我想[动作]，以便[目的]”描述
4. **功能详情**：每个功能点的细节、交互流程、边界情况
5. **信息架构**：页面层级与导航结构
6. **业务规则**：数据校验、权限控制、状态流转等
7. **非功能需求**：性能、安全、兼容性要求
8. **验收标准**：每个功能点的可验证测试条件
9. **时间计划建议**：功能拆分与分阶段上线建议

## 使用建议

- **先写用户故事再写功能**：避免“为了做功能而做功能”的陷阱
- **验收标准必须可验证**：应能明确判断通过/不通过
- **区分 MVP 与完整版**：标注首期必需与后续迭代功能
- **结合原型**：PRD 搭配低保真原型最有效，纯文字易产生误解
- **及时更新**：需求变更后应立即更新文档
- **关注异常流程**：不仅看主流程，也要考虑异常和边界情况
