...
tags: ["App Store", "Indie Hacking", "Publishing", "ASO", "Google Play", "collection:indie-hacker"]
featured: false
source: "skillhub"
---
...
|-----------|-----------|-------------|
| Name Length | 30 characters | 30 characters |
| Subtitle/Short Description | 30 characters | 80 characters |
| Keywords Field | 100 characters (separate field) | None (extracted from description) |
| Description Impact on Ranking | Lower | Higher (requires keyword embedding) |
| Screenshot Limit | Up to 10 | Up to 8 |
...
移动应用在 App Store / Google Play 的上架文案直接影响搜索排名与下载转化。ASO（应用商店优化）是独立开发者获取自然流量的核心手段。本技能帮助你生成完整的上架文案，并结合 ASO 策略提升可见度与下载量。
...
- **主名称**：不超过 30 字符（App Store 限制）
- 包含核心关键词且保持品牌识别度
- 2-3 个备选名称
- 命名策略分析

### 2. 副标题（App Store）/ 短描述（Google Play）

- **App Store 副标题**：不超过 30 字符
- **Google Play 短描述**：不超过 80 字符
- 补充应用名称未覆盖的信息
- 提供 2-3 个备选版本
...
- **App Store 描述**：不超过 4000 字符
- **Google Play 描述**：不超过 4000 字符
- 结构建议：
  - 第一段（最关键）：核心价值主张（默认只展示前几行）
  - 功能亮点：列表展示
  - 使用场景：典型使用场景描述
  - 用户评价（如有）：引用真实用户反馈
  - 结尾：下载 CTA + 联系方式
...
- **核心关键词**：直接描述应用功能（如“待办清单”）
- **长尾关键词**：更具体搜索短语（如“简单待办清单 app”）
- **竞品关键词**：竞品品牌词（谨慎使用，可能违规）
- **场景关键词**：与用户搜索场景相关（如“效率”“时间管理”）
...
| 名称长度 | 30 字符 | 30 字符 |
| 副标题/短描述 | 30 字符 | 80 字符 |
| 关键词字段 | 100 字符（独立字段） | 无（从描述提取） |
| 描述对排名影响 | 较低 | 较高（需嵌关键词） |
| 截图数量 | 最多 10 张 | 最多 8 张 |

### 转化率优化

- **前 3 张截图最重要**：大多数用户不会滑到后面
- **截图加文案**：带标题的截图转化更高
- **社会证明**：展示下载量、评分等指标
- **本地化**：不同市场定制语言与文化表达
- **频繁更新**：更新频率影响排名
...
- **研究竞品关键词**：借助 ASO 工具分析竞品关键词
- **前几行最关键**：商店默认只展示前几行
- **A/B 测试**：Google Play 支持 A/B 测试
- **持续迭代**：定期跟踪关键词排名并优化文案
- **避免关键词堆砌**：描述要自然流畅，过度堆砌会影响体验和审核
- **善用评分与评论**：鼓励用户评价提升转化
